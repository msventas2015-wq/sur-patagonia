...
declare
  ...
  v_policy_md5 text;
begin
  if to_regprocedure('private.l1_es_colaborador_activo_vivo_v1(uuid)') is null
     or to_regprocedure('public.l1_es_colaborador_activo_vivo_pub_v1()') is null then
    raise exception 'L1B_DEPENDENCIA_L1A_INEXISTENTE';
  end if;

  if to_regprocedure('public.contactos_insert_guard_v32()') is null then
    raise exception 'L1B_GUARD_BASE_INEXISTENTE';
  end if;
  ...
  if v_guard_md5 <> 'ddf2a6382b048a8a028c12864dbab0d1' then
    raise exception 'L1B_GUARD_BASE_INESPERADO: %', v_guard_md5;
  end if;
  ...
  if v_policy_md5 is distinct from '1bfe5da049ae5af97e58cd44ef1d440d' then
    raise exception 'L1B_POLICY_BASE_INESPERADA: %', coalesce(v_policy_md5, '<null>');
  end if;

  if not exists (
    select 1
    from pg_trigger t
    where t.tgrelid = 'public.contactos'::regclass
      and t.tgname = 'aa_contactos_insert_guard_v32'
      and not t.tgisinternal
      and t.tgenabled = 'O'
  ) then
    raise exception 'L1B_TRIGGER_BASE_INEXISTENTE_O_DESHABILITADO';
  end if;
end
$l1b_precheck$;
...
with check (
  ...
  and (((select auth.jwt()) -> 'app_metadata'::text) ->> 'rol'::text) = 'colaborador'::text
  and (((select auth.jwt()) -> 'app_metadata'::text) ->> 'tipo_acceso'::text) = 'activo'::text
  and public.l1_es_colaborador_activo_vivo_pub_v1()
...
begin
 ...
 elsif v_uid is not null and v_rol='colaborador' and v_tipo='activo'
       and private.l1_es_colaborador_activo_vivo_v1(v_uid) then
...
begin
  ...
  if v_check is null
     or position('l1_es_colaborador_activo_vivo_pub_v1' in v_check) = 0
     or position('c.tipo_acceso' in v_check) > 0 then
    raise exception 'L1B_POLICY_POSTCHECK_FALLIDO';
  end if;
  ...
  if position('private.l1_es_colaborador_activo_vivo_v1(v_uid)' in v_guard) = 0
     or position('c.tipo_acceso' in v_guard) > 0
     or position('v_rol=''admin''' in v_guard) = 0
     or position('Una consulta no puede tener propiedad y proyecto simultáneos' in v_guard) = 0 then
    raise exception 'L1B_GUARD_POSTCHECK_FALLIDO';
  end if;
  ...
  if not exists (
    ...
    raise exception 'L1B_TRIGGER_POSTCHECK_FALLIDO';
  end if;
end
$l1b_postcheck$;

select 'L1B_DERIVACION_ACTIVA_OK'::text as recibo;
